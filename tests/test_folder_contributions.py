...
def test_total_size(folder_tree):
    ...
    for f in ["d1", "d2"]:
        assert totals[f] == get_total_from_du(folder_tree / f), f"Wrong total size for {f}"

    assert totals["."] == get_total_from_du(folder_tree), f"Wrong total size for base folder"
...
def test_contribs(folder_tree):
    ...
    assert contribs["d1"] == get_total_from_du(folder_tree / "d1") - 1024, f"Wrong contrib size for d1"
    assert contribs["d2"] == get_total_from_du(folder_tree / "d2") - 1024, f"Wrong contrib size for d2"

    base_folder_size = get_total_from_du(folder_tree) - get_total_from_du(folder_tree / "d1") - contribs["d2"]
